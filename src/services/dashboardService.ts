...
export const getProfile = async (userId: number) => {
  const response = await axiosInstance.get("customers/getUserProfileData", {
    params: { userId }
  });
  return response.data;
};
...
export const updateDefaultAddress = async (
  ...
  const response = await axiosInstance.put("/customers", payload, {
    params: { customerId }
  });
  return response.data;
};
...
export const getAccessibleCustomersUsers = async(customerIds: number[])=>{
  const response = await axiosInstance.get("users", {
    params: { "companyName.in": customerIds.join(",") }
  });
  return response.data;
}
...
export const updateAddress = async (addressId: any, payload: any) => {
  const response = await axiosInstance.put("addresses", payload, {
    params: { addressId }
  });
  return response.data;
}
...
export const getGraphData = async (userId: number) => {
  const response = await axiosInstance.get("orders/GetGraphData", {
    params: { userId }
  });
  return response.data;
}
...
export const getWelcomeMessage = async (userId: number) => {
  const response = await axiosInstance.get("loginWelcomeMessages", {
    params: { userId }
  });
  return response.data[0];
}
...
export const fetchAttachment = async (referenceId: number) => {
  try {
    ...
    const response = await axiosInstance.get(
      "documentattachments/GetWCSAttchment",
      {
        params: { documentId: referenceId }
      }
    );
    return response;
